"""
handlers/client_history.py
==========================
Обработчик истории записей клиента за месяц
Показывает сводку по мастерам: имя/фамилия, услуги, количество записей
"""

from aiogram import Router, F
from aiogram.types import Message
from aiogram.fsm.context import FSMContext
from database import get_client_history_for_month  # ← Теперь работает!
...
async def show_client_history(message: Message, state: FSMContext):
    """
    Показывает историю записей клиента за последний месяц
    ...
    history = await get_client_history_for_month(message.from_user.id)
    
    # Формируем сообщение
    if not history:
        await message.answer(
            "У вас нет записей за последний месяц.",
            reply_markup=client_menu_keyboard()
        )
        return
    
    response = "📋 История ваших записей за месяц:\n\n"
    for item in history:
